package auction.repository.jdbctemplate;
...
public class JdbcTemplateItemRepository implements ItemRepository {
    ...
    public Item save(Item item) {
        String sql = "INSERT INTO items (artist, item_name, starting_price, auction_start, auction_end, category, owner_id, current_price) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public int update(Item item) {
        String sql = "UPDATE items SET artist = ?, item_name = ?, starting_price = ?, auction_start = ?, auction_end = ?, category = ?, owner_id = ?, current_price = ? WHERE item_id = ?";
