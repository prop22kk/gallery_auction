...
public class ItemController {
    ...
    private final gall_test.jytest.item.ItemService itemService;

    public ItemController(gall_test.jytest.item.ItemService itemService) {
        this.itemService = itemService;
    }

    // 1. 물품 추가
    @PostMapping
    public ResponseEntity<Item> createItem(@RequestBody gall_test.jytest.item.ItemRequest request) {
        Item item = new Item();
        ...
        item.setAuctionEnd(LocalDateTime.now().plusDays(7)); // 기본 7일
    ...
    public ResponseEntity<String> updateItem(@PathVariable("id") Long id,
        ...
        item.setCurrentPrice(request.getStartingPrice());
        item.setAuctionStart(LocalDateTime.now());
        item.setAuctionEnd(LocalDateTime.now().plusDays(7));
        item.setCategory(request.getCategory());
        item.setOwnerId(request.getOwnerId());
